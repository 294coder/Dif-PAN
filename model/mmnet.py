...
from model.module.nonlocal_block import *
...
class U_block(nn.Module):
    def __init__(self, in_channel=8, mid_channel=8):
        super(U_block, self).__init__()
        self.conv1 = ConvBlock(in_channel + 8, mid_channel, 5, 1, 2, activation='prelu', norm=None, bias=False)
        self.conv2 = RB(mid_channel, in_channel, 3, 1, 1, bias=False, activation='prelu', norm=None, n_resblocks=2)
        self.panhead = ConvBlock(1, 7, 5, 1, 2, activation='prelu', norm=None, bias=False)

    def forward(self, input, pan):
        pan_f = self.panhead(pan)  # 7
        pan_f = torch.cat((pan_f, pan), dim=1)  # 8
        x = torch.cat((input, pan_f), dim=1)  # inp_c + 8
        x = self.conv1(x)  # mid_channel
        U = self.conv2(x)  # inchannel
        mem_f = torch.cat((U, x, input), dim=1)  # in_c + mid_c + in_c
        return U, mem_f


class Hl_block(nn.Module):
    def __init__(self, in_channel=8, mid_channel=8, o_channel=8):
...
class vanilla_stage(nn.Module):
    def __init__(self):
        ...
        self.Hl = Hl_block(in_channel=4, mid_channel=4, o_channel=4)
        self.V = V_block(in_channel=4, mid_channel=4)
        self.Hp = Hp_block(in_channel=4, mid_channel=4, o_channel=4)
    ...
    def forward(self, H, L, pan):  # lms, ms, pan
        U, U_mem_out = self.U(H, pan)  # in_c, 2*in_c+mid_c
        H, Hl_mem_out = self.Hl(H, U, L, None)  # lms, U, ms
...
@register_model('mmnet')
class MMNet(BaseModel):
    def __init__(self, base_filter=None, args=None, num_channels=4, stage=3):
        ...
        ## main branch
        self.Uconv3 = C_RB(num_channels*3, num_channels, 3, 1, 1, bias=False, activation='prelu', norm=None, n_resblocks=2)
        self.H2SH_Hin = ConvBlock(num_channels, num_channels, 3, 2, 1, activation='prelu', norm=None, bias=False)
        self.H2SH_PANin = ConvBlock(1, num_channels, 3, 2, 1, activation='prelu', norm=None, bias=False)
        self.H2SH = NONLocalBlock2D(in_channels=num_channels, mode='embedded_gaussian')
        self.H2SH_out = C_RB(num_channels*2, num_channels, 1, 1, 0, bias=False, activation='prelu', norm=None, n_resblocks=1)
        self.H2SH_outup = nn.ConvTranspose2d(num_channels, num_channels, 3, 2, 1, 1,
                                             bias=False)  # ConvTranspose2d (in-1)*stride+outpad-2*pad+kernelsize
        self.Vconv3 = C_RB(num_channels*3, num_channels, 3, 1, 1, bias=False, activation='prelu', norm=None, n_resblocks=2)
    ...
    def _forward_implem(self, l_ms, bms, pan):  # ms, lms, pan
        H = bms
        ...
        SH, U_mem_f, Hl_mem_f, V_mem_f, Hp_mem_f = self.stage0(H, l_ms, pan)  # lms, ms, pan
        # 8 | 24 | 16 | 24 | 16
        for i in range(self.stage_num):
            ...
            U_mem_f = self.Uconv1(U_mem_f)  # 16
            V_mem_f = self.Vconv1(V_mem_f)  # 16

            u_h, u_c = self.cellU(U_mem_f, cur_state=[u_h, u_c])  # 16 | 16
            ...
            Hl_mem_f = self.Hlconv2(Hl_h)  # 8
            V_mem_f = self.Vconv2(v_h)
            Hp_mem_f = self.Hpconv2(Hp_h)  # 8

            ###main branch###
            H_f = torch.cat((SH, U_mem_f), dim=1)  # 8 + 16 = 24
            H_f = self.Uconv3(H_f)  # 8
    ...
    def train_step(self, ms, lms, pan, gt, criterion):
        sr = self._forward_implem(ms, lms, pan)
