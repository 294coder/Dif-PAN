...
class Convlutioanl(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv = nn.Conv2d(
            in_channel, out_channel, kernel_size=3, padding=0, stride=1
        )
...
class Convlutioanl_out(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv = nn.Conv2d(
            in_channel, out_channel, kernel_size=1, padding=0, stride=1
        )
...
class build_channel_branch(nn.Module):
    def __init__(self, channel, nonlinearity="linear", reduction=8, kernel_size=3):
        ...
        self.fc = nn.Sequential(
            nn.Conv2d(channel, 8, 1), nn.ReLU(inplace=True), nn.Conv2d(8, channel, 1)
        )
...
class DDFPack(nn.Module):
    def __init__(
        ...
        self.spatial_branch = build_spatial_branch(in_channels)
...
class PatchEmbed(nn.Module):
    def __init__(
        self, img_size=120, patch_size=4, in_chans=6, embed_dim=96, norm_layer=None
    ):
        super().__init__()
...
class MODEL(BaseModel):
    ...
    def _forward_implem(self, ir, vi):

        convInput_A1 = self.convInput(ir)
        layer_A1 = self.DDF(convInput_A1)
        encode_size_A1 = (layer_A1.shape[2], layer_A1.shape[3])
        Transformer_A1 = self.basicLayer(layer_A1, encode_size_A1)

        convInput_B1 = self.convInput(vi)
        layer_B1 = self.DDF(convInput_B1)
        encode_size_B1 = (layer_B1.shape[2], layer_B1.shape[3])
        Transformer_B1 = self.basicLayer(layer_B1, encode_size_B1)

        conv = Transformer_A1 + Transformer_B1

        DTRM1 = self.DDF(conv)
        encode_size_DTRM1 = (DTRM1.shape[2], DTRM1.shape[3])
        Transformer_DTRM1 = self.basicLayer(DTRM1, encode_size_DTRM1)

        out = self.convolutional_out(Transformer_DTRM1)
        return out
    
    def train_step(self, ms, lms, pan, gt, criterion):
        fuse = self._forward_implem(pan, lms)
        loss = criterion(fuse, gt)
        return fuse, loss
    
    def val_step(self, ms, lms, pan) -> torch.Tensor:
        return self._forward_implem(pan, lms)
...
if __name__ == "__main__":
    ...
    ir = torch.randn(1, 1, 120, 120).cuda(1)
    vis = torch.randn(1, 1, 120, 120).cuda(1)
