...
class PreFusionUBlock(nn.Module):
    def __init__(self, planes):
        super(PreFusionUBlock, self).__init__()
        ...
        self.encode2 = nn.Sequential(
            nn.Conv2d(planes, 2 * planes, 3, 2, 1),
            nn.BatchNorm2d(2 * planes),
            nn.LeakyReLU()
        )
        self.decode2 = nn.Sequential(
            nn.Conv2d(2 * planes, 2 * planes, 3, 1, 1),
            nn.BatchNorm2d(2 * planes),
            nn.LeakyReLU()
        )
        self.decode1 = nn.Sequential(
            nn.Upsample(scale_factor=2),
            nn.Conv2d(2 * planes, planes, 3, 1, 1),
            nn.BatchNorm2d(planes),
            nn.LeakyReLU()
        )
        self.middle = conv3x3(2 * planes, 2 * planes)
    ...
    def forward(self, x):
        ...
        recon_x2 = self.decode2(xm) + x2
        recon_x1 = self.decode1(recon_x2) + x1
...
class ChannelFuseBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, num_res=1, stride=1, downsample=None):
        ...
        inplanes = 31
        self.conv_up = conv3x3(inplanes, planes, stride)  # prefusion
        self.epsilon = 1e-4
        ...
        for i in range(num_res):
            ...
            self.res_block.append(PreFusionUBlock(planes))
            ...
            if i < num_branches_pre:  # 原有分支
                ...
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(nn.Sequential(
                        ...
                        PanFormerEncoderLayer((num_channels_cur_layer[i], num_channels_pre_layer[i]), 4, 0.2, 2,
                                              attn_type='R'))
                    )
                else:
                ...
                if i + 1 - num_branches_pre > 0:
                    transfpn.append(nn.Sequential(
                        TransitionFPN(len(num_channels_cur_layer), down_inchannel=512)
                        )
                    )
    ...
    def _make_fuse_layers(self):
        ...
        for i in range(num_branches if self.multi_scale_output else 1):
            ...
            for j in range(num_branches):
                if j > i:
                    fuse_layer.append(
                        PanFormerEncoderLayer((num_inchannels[i], num_inchannels[j]), 8, 0.2, 2, attn_type='R')
                    )
                elif j == i:
                    fuse_layer.append(None)
                else:
                    if num_branches == 2:
                        fuse_layer.append(PanFormerEncoderLayer(num_inchannels[i], 8, 0.2, 2, attn_type='C'))
                    elif num_branches == 3:
                        fuse_layer.append(
                            PanFormerEncoderLayer((num_inchannels[i], num_inchannels[j]), 8, 0.2, 2, attn_type='R'))
...
class TransitionFPN(nn.Module):
    def __init__(self, num_branches_after_trans, down_inchannel=512):
        ...
        if self.num_branches == 2:
            self.b0_in_down = nn.Sequential(
                nn.Conv2d(down_inchannel, 128, 3, 2, 1),
                nn.BatchNorm2d(128),
                nn.ReLU()
            )
        ...
        if self.num_branches == 3:
            self.epsilon = 1e-4
            self.fuse_block_b0_b1 = PanFormerEncoderLayer((128, 64), 8, 0.4, 2, attn_type='R')
            self.b1_down = nn.Sequential(nn.Conv2d(128, 256, 3, 2, 1),
                                         nn.BatchNorm2d(256),
                                         nn.ReLU())
            self.cfs_layers = ChannelFuseBlock(256, 256)
            self.fuse_block_b1_b2 = PanFormerEncoderLayer((256, 128), 8, 0.2, 2, attn_type='R')
    ...
    def forward(self, x):
        ...
        if num_branches == 3:
            ...
            # 先算SA再下采样
            # PCFT:  torch.Size([1, 64, 32, 32]) torch.Size([1, 32, 64, 64])
            out = self.fuse_block_b0_b1(b1_in, b0_in)  # 1,128,32,32
...
@register_model('dcformer_reduce_c64')
class DCFormer_Reduce_C64(BaseModel):
    def __init__(self,
                 spectral_num,
                 mode="C",
                 channel_list=(64, (32, 64), (32, 64, 128)),
                 block_list=(4, (4, 4), (4, 4, 4)),
                 added_c=3
                 ):
        super(DCFormer_Reduce_C64, self).__init__()
        ...
        # stem net
        STEM_PLANES = 128
        self.conv1 = nn.Conv2d(init_channel, STEM_PLANES, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.bn1 = nn.BatchNorm2d(STEM_PLANES)
        ...
        self.bn2 = nn.BatchNorm2d(STEM_PLANES)
        self.relu = nn.ReLU(inplace=True)

        ################################################################################################
        # stage 1
        # convolution at early stage make training process stable
        self.layer1 = self._make_layer(block=Bottleneck, inplanes=STEM_PLANES, planes=self.channels_list[0],
    ...
    def forward(self, x, ly, my, sy):
        ...
        PCFT:  torch.Size([1, 64, 32, 32]) torch.Size([1, 32, 64, 64])
        '''
        x_list = self.stage2((x_list, sy))  # x_list[0] [b, 32, 64, 64], x_list[1] [b, 64, 32, 32], sy(ms) [1, c, 16, 16]
        # 第三个分支内blocks+PCFT
...
if __name__ == '__main__':
    ...
    net = DCFormer_ReduceTmp(31, 'C', added_c=3, channel_list=[128, [64, 128], [64, 128, 256]])
    ...
    print(flop_count_table(FlopCountAnalysis(net, (pan, lms, mms, ms)), max_depth=3))
