...
class DualCrossTransformerBlock(nn.Module):
    def __init__(
        ...
        self.cross1 = CrossSwinTransformerBlock(
            dim=dim,
        ...
        self.cross2 = CrossSwinTransformerBlock(
            dim=dim,
            input_resolution=input_resolution,
            num_heads=num_heads,
            window_size=window_size,
...
class RSTB(nn.Module):
    ...
    def forward(self, x, y, x_size):
        return (
            self.patch_embed(
                self.conv(self.patch_unembed(self.residual_group(x, y, x_size), x_size))
            )
            + x
        )
...
class DCT(BaseModel):
    def __init__(self, n_colors, upscale_factor, n_feats=180):
        ...
        self.fe_conv1 = torch.nn.Conv2d(
            in_channels=2 * n_feats, out_channels=n_feats, kernel_size=3, padding=3 // 2
        )
        self.fe_conv2 = torch.nn.Conv2d(
            in_channels=3 * n_feats, out_channels=n_feats, kernel_size=3, padding=3 // 2
        )
        self.fe_conv3 = torch.nn.Conv2d(
            in_channels=4 * n_feats, out_channels=n_feats, kernel_size=3, padding=3 // 2
        )
...
if __name__ == "__main__":
    ...
    print(flop_count_table(FlopCountAnalysis(model, (lms, pan))))
