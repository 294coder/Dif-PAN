                ...
                if j > i:
                    fuse_layer.append(
                        nn.Sequential(
                            ...
                            nn.Conv2d(num_inchannels[j], num_inchannels[i], 1, 1),
                        )
                    )
                elif j == i:
...
@register_model("dcformer_abla_only_xca")
class DCFormer_XCA(BaseModel):
    def __init__(
            self,
            spectral_num,
            mode="SUM",
        ...
        super(DCFormer_XCA, self).__init__()
...
if __name__ == "__main__":
    ...
    net = DCFormer_XCA(31, "C", added_c=3, block_list=[4, [4, 3], [4, 3, 2]]).cuda()
