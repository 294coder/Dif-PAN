...
class BasicBlock2(nn.Module):
    ...
    def __init__(self, inplanes,
        ...
        self.block = PanFormerEncoderLayer(inplanes, attn_type='N', mlp_drop=0.2, mlp_ratio=2)
            ...
            if i < num_branches_pre:  # 原有分支
                ...
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(nn.Sequential(
                        ...
                        PanFormerEncoderLayer((num_channels_cur_layer[i], num_channels_pre_layer[i]),
                                              attn_type='SCAM'))
                    )
                else:
    ...
    def _make_fuse_layers(self):
        ...
        for i in range(num_branches if self.multi_scale_output else 1):
            ...
            for j in range(num_branches):
                if j > i:
                    fuse_layer.append(
                        PanFormerEncoderLayer((num_inchannels[i], num_inchannels[j]),attn_type='SCAM')
                    )
                elif j == i:
                    fuse_layer.append(None)
                else:
                    if num_branches == 2:
                        fuse_layer.append(PanFormerEncoderLayer(num_inchannels[i], attn_type='N', mlp_ratio=2, mlp_drop=0.2))
                    elif num_branches == 3:
                        fuse_layer.append(
                            PanFormerEncoderLayer((num_inchannels[i], num_inchannels[j]), attn_type='SCAM'))
    ...
    def __init__(self, num_branches_after_trans, inchannels=0, outchannels=0, kernel_size=3, stride=1, padding=1):
        ...
        if self.num_branches == 3:
            ...
            self.b1_down = nn.Sequential(nn.Conv2d(64, 128, 1, 1),
                                         ...
                                         nn.BatchNorm2d(128),
                                         nn.ReLU())
            self.fuse_block_b0_b1 = PanFormerEncoderLayer((64, 32), attn_type='SCAM')
            self.fuse_block_b1_b2 = PanFormerEncoderLayer((128, 64), attn_type='SCAM')
...
@register_model('dcformer_sg_c32')
class DCFormer_SG_C32(BaseModel):
    def __init__(self,
                 spectral_num,
                 mode="C",
        ...
        super(DCFormer_SG_C32, self).__init__()
...
if __name__ == '__main__':
    ...
    net = DCFormer_SG_C32(31, 'C').cuda(1)
