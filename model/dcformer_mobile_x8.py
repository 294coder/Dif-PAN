...
class HighResolutionModule(nn.Module):
    ...
    def _make_one_branch(self, branch_index, block, num_blocks, num_channels, stride=1):
        ...
        for i in range(1, num_blocks[branch_index]):
            layers.append(
                block(
                    self.num_inchannels[branch_index], num_channels[branch_index], 4, 4
                )
            )
            ...
            for j in range(num_branches):
                if j > i:
                    ...
                    if num_branches == 2:
                        fuse_layer.append(
                            PanFormerEncoderLayer(
                                num_inchannels[i], 8, 0.2, 2, attn_type="C",
                            )
                        )
                    elif num_branches == 3:
                        fuse_layer.append(
    ...
    def __init__(
        ...
        if self.num_branches == 2:
            self.b0_in_down = nn.Sequential(
                ...
                nn.Conv2d(64, 64, 3, 4, 1, groups=64),
                nn.BatchNorm2d(64),
                nn.ReLU(),
            )
        ...
        if self.num_branches == 3:
            ...
            self.b1_down = nn.Sequential(
                ...
                nn.Conv2d(
                    128, 128, kernel_size=3, stride=2, padding=1, bias=False, groups=128
                ),
                nn.BatchNorm2d(128),
                nn.ReLU(),
            )
            self.fuse_block_b0_b1 = PanFormerEncoderLayer(
                (64, 32), attn_drop=0.0, mlp_drop=0.2, attn_type="Mo", ffn_type=None,
            )
            self.fuse_block_b1_b2 = PanFormerEncoderLayer(
                (128, 64), attn_drop=0.0, mlp_drop=0.2, attn_type="Mo", ffn_type=None
            )
...
@register_model("dcformer_mobile_x8")
class DCFormerMobile(BaseModel):
    def __init__(
            self,
            spectral_num,
            mode="C",
    ...
    def _forward_implem(self, x, ly, my, sy):
        ...
        if self.first_fuse == "SUM":
            x = x.repeat(1, self.spectrum_num, 1, 1)
            x = x + ly
    ...
    def patch_merge_step(self, ms, lms, pan, hisi=False, split_size=64):
        ...
        ms = F.interpolate(
            ms,
            size=(split_size // 8, split_size // 8),
            mode="bilinear",
            align_corners=True,
        )
        if hisi:
...
if __name__ == "__main__":
    ...
    net = DCFormerMobile(31, "C", block_list=(4, (4, 3), (4, 3, 2)))
    ...
    lms = torch.randn(1, 31, 128, 128)
    pan = torch.randn(1, 3, 128, 128)
