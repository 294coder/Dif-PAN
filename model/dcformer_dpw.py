...
class ChannelFuseBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, num_res=1, stride=1, downsample=None):
        ...
        self.conv_up = conv3x3(8, planes, stride)  # prefusion
        self.epsilon = 1e-4
...
class TransitionFPN(nn.Module):
    def __init__(self, num_branches_after_trans, inchannels=0, outchannels=0, kernel_size=3, stride=1, padding=1):
        ...
        if self.num_branches == 3:
            ...
            self.b1_down = nn.Sequential(nn.Conv2d(64, 128, 1, 1),
                                         nn.Conv2d(128, 128, kernel_size=3, stride=2, padding=1, bias=False, groups=128),
                                         nn.BatchNorm2d(128),
                                         nn.ReLU())
            #
            ...
            self.fuse_block_b0_b1 = PanFormerEncoderLayer((64, 32), 4, 0., 2, attn_type='cross')
            # self.fuse_block_b1_b2 = PanFormerEncoderLayer((128, 64), 8, 0., 2, attn_type='cross')
    ...
    def forward(self, x):
        ...
        if num_branches == 3:
            ...
            # print('===FPN===')
            out = self.fuse_block_b0_b1(b1_in, b0_in)
            # out = self.fuse_block_b1_b2(weight[1] * self.cfs_layers((self.b1_down(out), b2_in)),
            #                             weight[0] * b1_in)
...
@register_model('dcformer_dpw')
class DCFormer_DPW(BaseModel):
    def __init__(self, spectral_num, mode="SUM"):
        super(DCFormer_DPW, self).__init__()
            ...
            if i < num_branches_pre:  # 原有分支
                ...
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(nn.Sequential(
                        ...
                        # nn.Conv2d(num_channels_pre_layer[i],
                        #           num_channels_cur_layer[i], 1, 1),
                        # PanFormerEncoderLayer((num_channels_pre_layer[i], num_channels_cur_layer[i]), 2, 0., 2, attn_type='cross'))
                    )
                else:
                    transition_layers.append(None)
            else:
