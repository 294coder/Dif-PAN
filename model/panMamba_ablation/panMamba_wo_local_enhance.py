"""
Ablation study:
panMamba network without local enhancememt
...
class MambaInjectionBlock(nn.Module):
    def __init__(
        ...
        **mamba_kwargs,
        ...
        # setting prev_state_chan and skip_state_chan to None
        prev_state_chan, skip_state_chan = None, None
        ...
        ssm_local_conv, ssm_global_conv = None, None  # ssm_conv[0], ssm_conv[1]
        local_d_state, global_d_state = None, None  # d_states[0], d_states[1]
...
class UniSequential(nn.Module):
    ...
    def LEMM_enc_forward(self, 
        ...
        for i, mod in enumerate(self.mods):
            ...
            outp, local_state, global_state = mod(outp, cond, c_shuffle) # in_block states share
...
@register_model("panMamba_wo_lemm")
class ConditionalNAFNet(BaseModel):
    ...
    def _forward_implem(self, inp, cond, c_shuffle=False):
        ...
        for i, (decoder, up, enc_skip) in enumerate(zip(self.lemm_decoders, self.lemm_ups, lemm_encs[::-1])):
            # print(f'unisequenctial decoder {i}')
            x = up(x)
            x = torch.cat([x, enc_skip], dim=-1)
            # print(x.shape[-1], enc_skip.shape[1])
            x, states = decoder.LEMM_dec_forward(x, cond, c_shuffle)
...
if __name__ == "__main__":
    ...
    net = ConditionalNAFNet(
        ...
        ssm_ratios=[2,2,2],
        window_sizes=[8,8,8],
        ssm_enc_d_states=[[None, 32], [None, 32], [None, 32]],
        ssm_dec_d_states=[[None, 32], [None, 32], [None, 32]],
        ssm_enc_convs=[[None, 11], [None, 11], [None, 11]],
