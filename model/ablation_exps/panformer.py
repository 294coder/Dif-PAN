...
from model.ablation_exps.attention import (
    Attention,
    CrossAttention,
    CAttention,
    CAttentionLegacy,
...
class PanFormerEncoderLayer(nn.Module):
    def __init__(
        ...
        if attn_type == "self":  # self-attention
            self.attn = Attention(dim, num_heads, dim_head, dropout=attn_drop)
        elif attn_type == "cross":  # cross-attention for different q, k, v
            self.attn = CrossAttention(
                dim[0], dim[1], num_heads, False, attn_drop=attn_drop
            )
        elif attn_type == "C":  # Channel Attention from Restormer
            # bias is True in ablation
            self.attn = CAttention(
                dim, num_heads, bias=False, attn_drop=attn_drop)
            # self.attn = CAttentionLegacy(
            #     dim, num_heads, bias=False, attn_drop=attn_drop
            # )
        elif attn_type == "S":  # stop gradient attention
            self.attn = SGA(dim, num_heads, bias, attn_drop=attn_drop)
        elif attn_type == "D":  # kernel generation (spectrum injection)
            self.attn = DynamicAttentionConv(
                dim, num_heads, attn_drop=attn_drop)
            ...
            self.attn = NAFBlock(dim, FFN_Expand=mlp_ratio,
                                 drop_out_rate=attn_drop)
            ffn_type = None
        ...
        if ffn_type == "1d":  # for 1d(sequence) input
            self.ffn = Mlp(dim, int(mlp_ratio * dim),
                           act_layer=mlp_act, drop=mlp_drop)
...
class PanFormerDecoderLayer(nn.Module):
    def __init__(
        ...
        self.cross_attn = CrossAttention(
            dim, heads, dim_head, dropout=attn_drop)
        self.norm1 = norm_layer(dim)
        self.norm2 = norm_layer(dim)
...
class PanFormer(BaseModel):
    def __init__(
        ...
        self.ms_ntoken = (ms_size[0] // patch_size[0]) * \
            (ms_size[1] // patch_size[1])
    ...
    def __init__(
        ...
        for i in range(2):
            self.pix_shuffle.append(
                nn.Sequential(
                    nn.Conv2d(hidden_c, hidden_c * 4, 3,
                              1, 1), nn.PixelShuffle(2)
                )
            )
