...
class BasicBlock2(nn.Module):
    ...
    def __init__(
        ...
        r = inplanes // BASE_CHANNEL
        sz = BASE_SIZE // r
        ws = BASE_WINDOW_SIZE // r
    ...
    def forward(self, x):
        # print(x.shape, self.block.attn.window_size)
        out = self.block(x)
...
@register_model("dcformer_abla_only_mwa")
class DCFormerOnlyMWA(BaseModel):
    def __init__(
        self,
        spectral_num,
        mode="SUM",
        ...
        super(DCFormerOnlyMWA, self).__init__()
...
if __name__ == "__main__":
    ...
    net = DCFormerOnlyMWA(31, "C", added_c=3, block_list=[4, [4, 3], [4, 3, 2]]).cuda()
    # for n, m in net.named_modules():
    #     print(n, m)
    #     print('-'*20)

    # ms = torch.randn(1, 4, 16, 16).cuda()
    # mms = torch.randn(1, 4, 32, 32).cuda()
