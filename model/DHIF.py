...
class HSI_Fusion(nn.Module):
    def __init__(self, Ch, stages, sf):
        ...
        if self.sf == 4:
            self.BT = nn.Sequential(nn.ConvTranspose2d(Ch, Ch, kernel_size=6, stride=4, padding=1), nn.LeakyReLU())
            self.B  = nn.Sequential(nn.Conv2d(Ch, Ch, kernel_size=6, stride=4, padding=1), nn.LeakyReLU())
        elif self.sf == 16:
        ...
        # modified to 20
        if self.sf == 20:
            self.BT = nn.Sequential(nn.ConvTranspose2d(Ch, Ch, kernel_size=7, stride=4, padding=3),
                                    nn.LeakyReLU(),
                                    nn.ConvTranspose2d(Ch, Ch, kernel_size=6, stride=4, padding=1), nn.LeakyReLU())
            self.B = nn.Sequential(nn.Conv2d(Ch, Ch, kernel_size=7, stride=5, padding=3),
                                   nn.LeakyReLU(),
                                   nn.Conv2d(Ch, Ch, kernel_size=6, stride=4, padding=1), nn.LeakyReLU())
...
if __name__ == "__main__":
    ...
    img_size = 80
    scale = 20
    net = HSI_Fusion(ch, 4, scale).to(device)
    ms = torch.randn(1, 3, img_size, img_size).to(device)
    hs = torch.randn(1, ch, img_size//scale, img_size//scale).to(device)
    
    with torch.no_grad():
        import time
        t1 = time.time()
        for _ in range(10):
            sr = net._forward_implem(ms, hs)
