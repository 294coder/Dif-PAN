...
if __name__ == '__main__':
    ...
    image_size = 64
    device = 'cuda:0'
    unet = UNet(in_channel=image_n_channel * 2 + 1,
                        out_channel=image_n_channel,
                        image_size=image_size,
                        res_blocks=2,
                        attn_res=(8, 16),
                        channel_mults=(1, 2, 2, 4)).to(device)
    schedule = dict(
        schedule='cosine', n_timestep=3000, linear_start=1e-4, linear_end=2e-2, cosine_s=8e-3
    )
    diffusion = GaussianDiffusion(unet, 64, 8, schedule_opt=schedule)
    noise_schedule = NoiseScheduleVP(betas=diffusion.betas)
    model_fn = model_wrapper(
        unet,
        noise_schedule,
        guidance_type='classifier-free',
        guidance_scale=1.,
        condition=torch.randn(1, 9, 64, 64).cuda(),
        model_kwargs=dict(interm_fm=True)
    )

    solver = DPM_Solver(model_fn, noise_schedule)

    solver.sample(
        torch.randn(1, 8, 64, 64).cuda(),
        steps=20
    )
